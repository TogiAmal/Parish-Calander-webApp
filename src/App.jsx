...
import { 
    collection, addDoc, query, where, onSnapshot, deleteDoc, doc, setDoc, getDoc, updateDoc 
...
const InputWithIcon = ({ icon, ...props }) => (
  <div style={{ position: 'relative', marginBottom: '20px', width: '100%', boxSizing: 'border-box' }}>
    ...
    <input {...props} style={{
      width: '100%', maxWidth: '100%', padding: '12px 15px 12px 45px', border: 'none', borderRadius: '50px', background: 'rgba(255, 255, 255, 0.9)', fontSize: '16px', outline: 'none', boxSizing: 'border-box', color: '#333'
    }} />
  </div>
);
...
function App() {
  ...
  const handleRegister = async (e) => { e.preventDefault(); setLoginError(''); if (username.length < 3) { setLoginError("Too short"); return; } try { const d = await getDoc(doc(db, "usernames", username.toLowerCase())); if (d.exists()) { setLoginError("Taken"); return; } const u = await createUserWithEmailAndPassword(auth, email, password); await updateProfile(u.user, { displayName: displayName || username, photoURL: `https://ui-avatars.com/api/?name=${(displayName||username).replace(' ','+')}&background=1a73e8&color=fff` }); await setDoc(doc(db, "usernames", username.toLowerCase()), { email: email, uid: u.user.uid }); await signOut(auth); setIsRegistering(false); setSuccessMsg("Created!"); } catch (e) { setLoginError(e.message); } };
  const handleForgotPassword = async (e) => { e.preventDefault(); try { await sendPasswordResetEmail(auth, email); setSuccessMsg("Sent!"); setIsForgotPassword(false); } catch (e) { setLoginError(e.message); } };
  const handleUpdateProfile = async () => { try { if (displayName) await updateProfile(user, { displayName }); if (newPassword) await updatePassword(user, newPassword); alert("Updated!"); setShowEditProfile(false); } catch (e) { alert(e.message); } };
  const handleGoogleLogin = async () => { try { await signInWithPopup(auth, googleProvider); } catch (e) { console.error(e); } };
  const handleLogout = () => { if(confirm("Log out?")) { signOut(auth); } };
  ...
  const handleDeleteEvent = async () => {
      if (newEvent.id && confirm("Delete this event?")) {
          setModalOpen(false);
        ...
        <form onSubmit={isForgotPassword ? handleForgotPassword : (isRegistering ? handleRegister : handleUsernameLogin)}>
          ...
          {!isForgotPassword && (
            <div style={{ position: 'relative', marginBottom: '20px', width: '100%', boxSizing: 'border-box' }}>
              ...
              <input type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required style={{width: '100%', maxWidth: '100%', padding: '12px 45px 12px 45px', border: 'none', borderRadius: '50px', background: 'rgba(255, 255, 255, 0.9)', fontSize: '16px', outline: 'none', boxSizing: 'border-box', color: '#333'}} />
